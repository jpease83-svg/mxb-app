...
/// Join the control plane and remember the token.
///
/// Public deployments ignore `code`; private deployments still use it for a one-use invite.
#[tauri::command]
...
async fn pull_rosters(
    ...
    // The catalog is what makes solo viewing real: paint owners only need to have published
    // once, not keep MXB App running in the same race. It is capped server-side, and the
    // pull de-duplicates it with the exact live roster before downloading anything.
    let mut pull_keys = keys;
    pull_keys.push(paintsync::CATALOG_SCOPE.to_string());
    let outcome = paintsync::pull(&cfg, &cfg.cp_token, &pull_keys)
        .await
        .map_err(|e| format!("{e:#}"))?;
